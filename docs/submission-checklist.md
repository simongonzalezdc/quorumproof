...
- **Demo video:** 2-min script locked (`docs/demo-script.md`). Production reassigned by CEO order to the **kinocut + Hyperframes lane** (screen-recorded live demo, both themes). Upload (YouTube unlisted) is a CEO act; the URL slots into the form field below. This repo's `docs/demo-video.*` is owned by that lane.
